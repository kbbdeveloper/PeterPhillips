 ...
 */
class ACA_Pods_Setting_Field extends AC_Settings_Column {
	...
	private function get_link_create_pod_field() {
		return add_query_arg( array( 'page' => 'pods-add-new' ), admin_url( 'admin.php' ) );
	}
	...
	private function get_field_types() {
		...
		if ( $fields = $this->column->get_pod_fields() ) {
			foreach ( $fields as $field ) {
				$options[ $field['name'] ] = $field['label'] ? $field['label'] : __( 'empty label', 'codepress-admin-columns' );
			}
		}
